...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    layer7_trans_conv = tf.layers.conv2d_transpose(layer7_conv, num_classes, 4,
                                             strides=(2, 2),
                                             padding='same',
                                             kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                             kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # skip connection
    skip1_out = tf.add(layer7_trans_conv, layer4_conv)

    # upsample
    skip1_trans_conv = tf.layers.conv2d_transpose(skip1_out, num_classes, 4,
                                             strides=(2, 2),
                                             padding='same',
                                             kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                             kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # skip connection
    skip2_out = tf.add(skip1_trans_conv, layer3_conv)

    # upsample
    last_layer = tf.layers.conv2d_transpose(skip2_out, num_classes, 16,
                                               strides=(8, 8),
                                               padding='same',
                                               kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                               kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
